...
vector< vector<float> > normalize(vector< vector <float> > &grid) {
	...
	for (size_t r = 0; r < grid_h; r++) {
		for (size_t c = 0; c < grid_c; c++ ) {
			sumOfElements += grid[r][c];
		}
	}
	for (size_t r = 0; r < grid_h; r++) {
		for (size_t c = 0; c < grid_c; c++ ) {
			newGrid[r][c] = grid[r][c] / sumOfElements;
		}
	}
	return newGrid;
}
		...
		for (int j = 0; j < grid_w; j++ ) {
			...
			for (int dx = -1; dx < 2; dx++ ) {
				for (int dy = -1; dy < 2; dy++) {
					...
					new_i = HELPERS_modulo((i + dy), grid_h);
					new_j = HELPERS_modulo((j + dx), grid_w);
